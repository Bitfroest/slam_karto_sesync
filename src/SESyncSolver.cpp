...
void SESyncSolver::AddNode(karto::Vertex <karto::LocalizedRangeScan> *pVertex) {
    ...
    if (debug_) {
        vertexListStream_ << "VERTEX_SE2 " << pVertex->GetObject()->GetUniqueId() << " " << odom.GetX() << " "
                          << odom.GetY() << " " << odom.GetHeading() << "\n";
    }
...
void SESyncSolver::AddConstraint(karto::Edge <karto::LocalizedRangeScan> *pEdge) {
    ...
    if (debug_) {
        ...
        edgeListStream_ << "EDGE_SE2 " << sourceID << " " << targetID << " " << diff.GetX() << " " << diff.GetY() << " "
                        << diff.GetHeading() << " "
...
void SESyncSolver::getGraph(std::vector <Eigen::Vector2d> &nodes,
